...
const InTheNews = () => {
  return (
    ...
    <div className="h-[5px] bg-gradient-to-r from-rose-600 to-rose-900"></div>
      <h3 className="bg-white bg-gradient-to-r from-rose-600 to-rose-900 bg-clip-text text-transparent p-6 text-2xl md:text-3xl text-center lg:text-5xl font-black uppercase">
    In the news</h3>
    <div className="flex flex-wrap w-full bg-white text-gray-600 font-ysabeau">
      ...
      {newsArray.map((news, index) => (
        <div key={index} className="w-full sm:w-1/2 md:w-1/2 lg:w-1/2">
          <div className="hover:shadow p-6 h-full flex flex-col justify-evenly">
            ...
            <h2 className="text-xl sm:text-4xl font-black mb-4 text-gray-600">{news.title}</h2>
            <a href={news.link} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:underline">
              Read More
            </a>
          </div>
        </div>
